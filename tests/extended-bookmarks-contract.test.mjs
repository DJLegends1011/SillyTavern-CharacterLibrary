...
test('Local Backup detail action is compact on exactly the original five providers', () => {
    ...
    assert.equal((sharedCssSource.match(/\.browse-meta-action\s*\{/g) ?? []).length, 1);
...
test('DataCat and Janny preserve their provider actions beside one Local Backup action', () => {
    assert.doesNotMatch(datacatBrowseSource, /^<<<<<<<|^=======$|^>>>>>>>/m);
    assert.doesNotMatch(jannyBrowseSource, /^<<<<<<<|^=======$|^>>>>>>>/m);

    assert.match(
        datacatBrowseSource,
        /id="datacatFolderBtn"[\s\S]{0,300}<\/button>[\s\S]{0,80}\$\{datacatBookmarks\.renderMetaAction\(\)}/,
    );
    assert.match(
        jannyBrowseSource,
        /id="jannyBookmarkBtn"[\s\S]{0,300}<\/button>[\s\S]{0,80}\$\{jannyBookmarks\.renderMetaAction\(\)}/,
    );

    for (const source of [datacatBrowseSource, jannyBrowseSource]) {
