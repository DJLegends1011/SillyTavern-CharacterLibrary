...
Layout / Chrome (defined in library.css; no !important needed):
  ...
  .linked-lorebook-card  Edit tab's lorebook-links card (primary bar + additional row, divided). Card bg rgba(var(--accent-rgb),0.05) follows accent; .linked-lorebook-tile is the leading 36px icon tile (accent-alpha bg; icon var(--cl-success-pale) linked, var(--text-faint) unlinked); .linked-lorebook-meta is the muted second line.
  .aux-lorebook-pill     Additional-lorebook pill in the card's .linked-lorebook-aux row. Accent-alpha bg rgba(var(--accent-rgb),0.08) follows accent; the book icon and the row label icon ride var(--cl-info-pale) (the additional-relationship hue, matching the manager's badges); the X hover rides var(--cl-error-rgb). .linked-aux-tag is the small "Additional" tag on the read view's nested book sections, same accent-alpha coupling.
...
Lorebooks (defined in modules/lorebook-manager.css; NEEDS !important for theme overrides). A self-contained World-Info editor plus AI-generation suite. It reuses the canonical shells (.modal-glass, .cl-modal-content) and the token system, but carries its own cohesive entry/button chrome (the way character-creator does). Accent-active states follow var(--accent-rgb) and retheme automatically EXCEPT where flagged:
  ...
  .lb-linked-chip        Avatar+name chip for a character using the open lorebook (editor header). ".lb-linked-chip.aux" is the additional-book variant: dashed border + accent-alpha bg (follows accent).
  .lb-rel-badge          Relationship badge inside a chip. ".lb-primary-badge" rides var(--cl-success-pale), ".lb-aux-badge" var(--cl-info-pale); both follow a status-tier retheme, not accent. Same hues as the link-picker "Linked here"/"Additional here" statuses.
...
Inputs / Modals / Dropdowns:
  ...
  .cl-confirm-overlay      The generic confirm dialog built by window.showConfirm({title, message, danger, icon, content}). Left intent-badge layout: .cl-confirm-main (badge + text row), .cl-confirm-badge (42px circle, accent-tinted by default; add-class .cl-confirm-danger on the overlay switches it to red via rgba(var(--cl-error-rgb)) bg + var(--cl-error-bright) icon), .cl-confirm-text (the column: an h3 title + #clConfirmMessage), and .confirm-modal-footer (Cancel + Confirm action-btns; the confirm button gets .danger when danger:true). Structured bodies (the showConfirm content block array) render inside #clConfirmMessage as: .cl-confirm-p (paragraphs), .cl-confirm-stats > .cl-confirm-stat (pill chips with accent icon, .cl-confirm-stat-label faint + .cl-confirm-stat-value bold), .cl-confirm-list (bullets), .cl-confirm-kv > .cl-confirm-kv-row (.cl-confirm-kv-key faint label + value, mono values as <code>), and .cl-confirm-note with tone variants -info (accent-tinted) / -warning (--cl-warning) / -danger (--cl-error). All tones ride status tokens so they follow a retheme. Badge and title colors come from --accent / --cl-error / --text-primary tokens. Toggled via the .hidden class on #clConfirmOverlay.
...
5. Module CSS files load AFTER the custom CSS <style> tag. Anything in modules/ needs !important to win source-order. Module CSS files (17): custom-css, recommender, playlists, card-update, batch-tagging, tags-manager, batch-transfer, character-creator, character-versions, chats, css-assistant, gallery-sync, gallery-viewer, multi-select, context-menu, lorebook-manager, media-download-queue. Provider browse CSS files (9 with class prefixes): browse-shared, chub-browse, chartavern-browse, pygmalion-browse, datacat-browse, saucepan-browse, botbooru-browse, charavault-browse, janitorai-browse. (Wyvern's CSS file holds a single [data-select-id] rule and otherwise inherits .browse-*. Janny has no CSS file; inherits from browse-shared. janitorai-browse holds the .janitorai-check-list / .janitorai-check / .janitorai-check-label / .janitorai-check-detail endpoint report used in Settings > Online; everything else inherits .browse-* / .chub-view-*.) The in-modal preview notices (locked/hidden/auth-required/extraction-unavailable definition states) across all providers use the shared .browse-notice component in browse-shared.css (.browse-notice-{locked,hidden,warning,error,info}, .browse-notice-cta for the centered empty-state layout, .browse-notice-actions for its buttons); no provider hand-rolls that banner. Class-prefix map for the modules: .ccss-* (custom-css), .recommender-*, .pl-* (playlists), .card-update-*, .bt-* (batch-tagging), .tgm-* (tags-manager: the Tags Manager modal - windowed tag list rows .tgm-row, import-rule rows .tgm-rule-row, case-dupe pills .tgm-dupe-pill; accent-alpha hovers follow var(--accent-rgb)), .btx-* (batch-transfer: the bundle export/import modal), .creator-* (character-creator), .vt-* (character-versions), .chat-*, .css-assistant-*, .gallery-sync-*, .gv-* (gallery-viewer), .multi-select-*, .cl-context-menu-*, .lb-* (lorebook-manager), .mdq-* (media-download-queue), .browse-*, .chub-*, .ct-* (chartavern), .pyg-* (pygmalion), .datacat-*, .saucepan-*, .botbooru-*, .cv-* (charavault), .janitorai-*. For library.css selectors (.char-card, .topbar, .glass-btn, .toast, .settings-*, .view-toggle, .tab-pane, .modal-glass, .modal-sidebar, .favorite-indicator, etc.), source order favors custom CSS so no !important is needed.
